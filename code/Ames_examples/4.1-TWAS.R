...
# mkdir
dir.create("RESULT/4.1-TWAS", recursive = T)
...
KinMat <- read.csv("RAWDATA/Kinship/GAPIT.Kin.VanRaden_LD_0.1.csv", header = FALSE, stringsAsFactors = F)
...
for ( i in 1:length(trait.all) ) {
   ...
   file.TwasRes <- paste0("RESULT/4.1-TWAS/TwasResult_", ver, "_", ref, "_", trait, ".csv")
