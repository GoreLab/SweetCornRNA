...
# lim <- range(c(T.min, T.max))
# plot(x = 1:length(T.min), y = T.min, ylim = lim, pch = 20, col = 2, ylab = "Temp", xlab = "day")
# points(x = 1:length(T.min), y = T.min, type = "l", lty = 1, col = 2)
# points(x = 1:length(T.min), y = T.max, pch = 18, col = 4)
# points(x = 1:length(T.min), y = T.max, type = "l", lty = 1, col = 4)
