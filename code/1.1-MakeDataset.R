...
# gene-filtering
zero.count <- apply(dat.matrix.samples, 2, FUN = function(x){sum(x == 0)} )
N <- nrow(dat.matrix.samples)
zero.proportion <- zero.count / N
tf.gene <- zero.proportion < 0.9; table(tf.gene)
dat.matrix.samples.filt <- dat.matrix.samples[, tf.gene]

# visualize the filter
png(figname, width = 400, height = 300)
hist(x = zero.proportion, xlim = c(0, 1), xlab = "Proportion of zero rlog2 value",
     main = paste0("Gene Filtering: ", ref, "_", ver))
abline(v = 0.9, col = "red", lty = 2)
dev.off()

# summary number
n.gene.removed <- sum(!tf.gene)
n.gene.retained <- sum(tf.gene)

# save
write.csv(dat.matrix.samples.filt, file = filename.data)
...
text.summary <- c(paste0("Summary numbers for the ", ref, "-mapped, ", ver, "dataset"), 
                  ...
                  paste0("number of filtered genes: ", n.gene.removed),
                  paste0("number of retained genes: ", n.gene.retained),
                  "",
                  paste0("Therefore, the final expression data has ",
                         nrow(dat.matrix.samples.filt),
                         " samples and ", 
                         ncol(dat.matrix.samples.filt), 
                         " genes"))
