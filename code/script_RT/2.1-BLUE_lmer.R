...
s <- as.numeric(args[1]) # start
e <- as.numeric(args[2]) # end
...
for ( i in s:e ) {
  ...
  if ( i == 1 ) {
    df.save <- df.fit[, -1]
    write.csv(df.save, file = paste0(dir.save, "/DataFrame_BLUE.csv"))
  }
  ...
  ) # evaluate function error by try function
  
  # when no error, get result
  if ( class(res.try) != "try-error" ) {
    ...
    write.csv(df.save, 
              file = paste0(dir.save, "/BLUE_", formatC(i, digits = 5, flag = "0"), ".csv"),
              row.names = FALSE)
    
    # write warning message
    wrn.message <- unlist(res@optinfo$conv$lme4$messages)
    write(wrn.message,
          file = paste0(dir.save, "/warning_", formatC(i, digits = 5, flag = "0"), ".txt"))
...
write.csv(df.save, 
          file = paste0(dir.save, "/num_error_from_", s, "_to_", e, ".csv"),
          row.names = F)
