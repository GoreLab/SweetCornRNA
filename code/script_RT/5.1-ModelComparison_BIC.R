...
# mkdir
dir.save <- "RESULT/5.1-ModelComparison_BIC"
dir.create(dir.save, recursive = T)

# file I/O
file.in.PeerResid <- paste0("RESULT/4.4-QuickOutlierRemoval/PeerResiduals_RmOut_UseOptFact_BLUE_lmer_alpha_5.txt")
...
# save result
write.csv(res.BIC.all, file = paste0(dir.save, "/all_BIC_result.csv"), row.names = F)
...
write.csv(df.opt.model, file = paste0(dir.save, "/OptimalModel_BIC.csv"), row.names = F)
