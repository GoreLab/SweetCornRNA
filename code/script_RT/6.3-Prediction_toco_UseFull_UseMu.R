...
# mkdir
dir.save <- "RESULT/6.3-Prediction_toco_UseFull_UseMu"
...
file.in.kinship <- "RAWDATA/SNPs_and_kinship/UseAll/Kinship_from_GAPIT_UseFull.csv"
...
tf <- is.na(PeerResidMat.sc.final.sc)
PeerResidMat.sc.final.sc[tf] <- 0
...
for ( i in 1:length(trait.all) ) {
   ...
   write.csv(df.CV.res.mGRM, row.names = F,
             file = paste0(dir.save, "/PredictionResult_", trait, "_mGRM.csv"))
   write.csv(df.CV.res.tGRM, row.names = F,
             file = paste0(dir.save, "/PredictionResult_", trait, "_tGRM.csv"))
   write.csv(df.CV.res.mGRM.tGRM, row.names = F,
             file = paste0(dir.save, "/PredictionResult_", trait, "_mGRM.tGRM.csv"))
}
...
cor.mGRM <- as.numeric(cor(Pheno.Sub[, trait], df.CV.res.mGRM[, -1], use = "p"))
cor.tGRM <- as.numeric(cor(Pheno.Sub[, trait], df.CV.res.tGRM[, -1], use = "p"))
cor.mGRM.tGRM <- as.numeric(cor(Pheno.Sub[, trait], df.CV.res.mGRM.tGRM[, -1], use = "p"))
df.fig <- reshape2::melt(cbind("mGRM" = cor.mGRM, "tGRM" = cor.tGRM, "mGRM.tGRM" = cor.mGRM.tGRM))
df.fig <- df.fig[, 2:3]
colnames(df.fig) <- c("method", "correlation")
p <- ggplot(df.fig, aes(x = method, y = correlation))
p <- p + geom_boxplot()
p <- p + ggtitle(trait)
p
