...
v4 <- read_csv("./data/gene_model_xref_v4.csv", skip = 4, skip_empty_rows = T, col_names = T) %>%
...
apriori_christine21 <- read_excel("./data/koab032_supplementary_data/tpc.01048.2020-s02.xlsx",
                                  skip = 4) %>%
...
apriori_christine17 <- read_excel("./data/TPC2017-LSB-00475R1_Supplemental_Data_Set_1.xlsx",
                                  skip = 3) %>%
...
apriori_matt21 <- read_excel("./data/Baseggio2021_supplmenal_material/Tables/Table S4.xlsx", skip = 1) %>%
...
apriori_whitt20 <- read_excel("./data/pld3272-sup-0001-tables1.xlsx", sheet = "Z.mays") %>%
...
apriori_di21 <- read_excel("./data/Supplemental Table Sx. a priori gene list_DW_20210212.xlsx", skip = 2) %>%
  rename(`RefGen_v4 Gene ID` = `Gene ID (RefGen v4)`) %>%
  mutate(Paper = "Wu_21",
         Trait_group = "?",
         Annotation = paste(`Gene name (MaizeGDB)`, `Gene annotation`, sep = "; "))

apriori_ziegler17 <- read_excel("./data/Baseggio2021_supplmenal_material/Tables/Table S7.xlsx", skip = 1) %>%
