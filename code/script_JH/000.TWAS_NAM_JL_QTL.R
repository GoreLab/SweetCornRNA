...
NAM_GWAS_uplifted <- read_excel("./data/apriori/Supplemental Table Sx. NAM_GWAS_uplifted.xlsx", skip = 1)
NAM_JL_uplifted.1 <- read_excel("./data/apriori/Supplemental Table Sx. NAM_JL_uplifted.xlsx", skip = 1)
...
NAM_JL_GWAS_carotenoids.1 <- read_excel("./data/apriori/koab032_supplementary_data/tpc.01048.2020-s04.xlsx", skip = 13)
...
NAM_JL <- NAM_JL_rbound %>%
  ...
  dplyr::select(QTL_ID,
                `Common Support Interval ID`, Phenotype, #TODO
                `NAM JL-QTL traits`,
...
write.csv(NAM_JL, "./output/NAM_JL_toco_carot_joined.csv", row.names = F)
