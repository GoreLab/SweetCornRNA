...
dir.blue.in.lmer <- "PH207/2.1-PH207-BLUE_lmer"
#dir.blup.in.lmer <- "PH207/2.2-PH207-BLUP_lmer"
dir.save <- "PH207/3.1-PH207-MergeBlueBlup_lmer"
...
for (i in num.get) {
  ...
  file.i <- paste0(dir.blue.in.lmer, "/PH207_BLUE_", num.i, ".csv")
...
write.csv(df.var, file = paste0(dir.save, "/PH207_EstVarComp_BLUE_lmer.csv"), row.names = F)
...
write.csv(df.BLUE, file = paste0(dir.save, "/PH207_BLUE_lmer.csv"), row.names = F)
...
write.csv(df.others, file = paste0(dir.save, "/PH207_OtherEffects_BLUE_lmer.csv"), row.names = F)
...
# for (i in num.get) {
#   num.i <- formatC(i, digits = 5, flag = "0")
#   file.i <- paste0(dir.blup.in.lmer, "/PH207_BLUP_", num.i, ".csv")
#   dat.i <- read.csv(file.i)
#   if ( i == num.get[1] ) {
#     df.BLUP.all <- dat.i
#   } else {
...
# write.csv(df.var, file = paste0(dir.save, "/PH207_EstVarComp_BLUP_lmer.csv"), row.names = F)
#
...
# write.csv(df.BLUP, file = paste0(dir.save, "/PH207_BLUP_lmer.csv"), row.names = F)
#
...
# write.csv(df.others, file = paste0(dir.save, "/PH207_OtherEffects_BLUP_lmer.csv"), row.names = F)
