...
# mkdir
dir.save <- "RESULT_NoHarvDate/9.6-ModelComparison_BIC_mutant_type"
...
df.covariate <- df.covariate[df.covariate$use.mu == "no", ] # NO NEED TO USE MU
...
# ---------------------------------------------------------------------------- #
# update -- remove su1sh2 mutant type
mu.type <- design.X.full[, "sh2"]
mu.type[design.X.full[, "su1sh2"] == 1] <- NA # set the 17 lines as NA
design.X.full <- cbind(design.X.full, "endosperm.mutation" = mu.type)
# ---------------------------------------------------------------------------- #

# target phenotype data
trait <- "endosperm.mutation"
pheno <- design.X.full[, trait]
...
p <- p + facet_grid(~ use.kin)
p <- p + scale_x_continuous(name = "Number of PCs", breaks = 0:11)
p <- p + ggtitle(paste0("Model Comparison: trait = ", trait))
ggsave(filename = paste0(dir.save, "/Model_Comparison_BIC_", dat, "_", trait, ".png"),
       p, width = 8, height = 8)

# save result
write.csv(res.BIC, file = paste0(dir.save, "/all_BIC_result_", dat, ".csv"), row.names = F)

# save best model
j <- which.min(res.BIC$BIC)
res.BIC.best <- res.BIC[j, ]
vec.n.PC <- res.BIC.best$n.PC
tf.vec.kin <- res.BIC.best$use.kin == "Use Kinship"
tf.vec.mu <- res.BIC.best$use.mu == "Use Endospearm Mutant"
df.opt.model <- data.frame("trait" = "endosperm.mutation",
                           "n.PC" = vec.n.PC,
                           "use.kin" = tf.vec.kin,
                           "use.mu" = tf.vec.mu)
