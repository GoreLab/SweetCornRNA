...
cutoff <- as.numeric(args[2]) # numeric value for the MAD threshold

# mkdir
dir.save <- "RESULT/1.3-OutlierRemoval_and_Imputation"
dir.create(dir.save)

# file to load
filename.data <- paste0("RESULT/1.2-GeneFiltering/ExpressionData_Filtered_", ref, ".csv")
...
fwrite(MAD.Dat, file = paste0(dir.save, "/MAD_matrix_", ref, ".csv"))
...
df.save.01 <- data.frame("GeneID" = names(n.rm),
                         ...
                         row.names = NULL)
fwrite(df.save.01, file = paste0(dir.save, "/number_of_outliers_", ref, ".csv"))
...
fwrite(df.save.02, file = paste0(dir.save, "/summary_table_", ref, ".csv"))
...
fwrite(ExprDat.Imp, file = paste0(dir.save, "/ExpressionData_For_BLUE_", ref, ".csv"))
