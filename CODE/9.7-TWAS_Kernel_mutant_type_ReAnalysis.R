...
# mkdir
dir.save <- "RESULT_NoHarvDate/9.7-TWAS_mutant_type"
...
file.in.OptModel <- paste0("RESULT_NoHarvDate/9.6-ModelComparison_BIC_mutant_type/OptimalModel_BIC_", dat, ".csv")
if ( ref == "B73" ) {
...
# # make kernel phenotype
# M <- model.matrix(~ mutation.vec - 1)
# df.pheno <- data.frame("Sample.ID" = Pheno.All$Sample.ID,
#                        "sh2" = M[, "mutation.vecsh2"], 
#                        "su1" = M[, "mutation.vecsu1"],
#                        "su1sh2" = M[, "mutation.vecsu1sh2"])

# ---------------------------------------------------------------------------- #
# update -- remove su1sh2 mutant type
M <- model.matrix(~ mutation.vec - 1)
df.pheno <- data.frame("Sample.ID" = Pheno.All$Sample.ID, 
                       "endosperm.mutation" = NA)
df.pheno$endosperm.mutation[mutation.vec == "sh2"] <- 1
df.pheno$endosperm.mutation[mutation.vec == "su1"] <- 0
# ---------------------------------------------------------------------------- #

# loop for all three ways
trait.all <- c("endosperm.mutation")
for ( i in 1:length(trait.all) ) {
   ...
   if ( use.mu == "yes" ) {
      ...
      pheno <- df.pheno[, c("Sample.ID", trait)]
   }
