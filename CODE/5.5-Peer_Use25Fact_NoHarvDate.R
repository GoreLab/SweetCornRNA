...
file.out.pdf <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_plotModel.pdf")
file.out.factors <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_factors.txt")
file.out.weights <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_weights.txt")
file.out.precision <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_precision.txt")
file.out.residuals <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_residuals.txt")
file.out.seed <- paste0("RESULT_NoHarvDate/5.1-Peer_Use25Fact/PeerResult_Use25Fact_", dat, "_", ref, "_seed.txt")

# mkdir
dir.create("RESULT_NoHarvDate/5.1-Peer_Use25Fact")

# load data
ex_clean.raw <- fread(file = file.input, head = TRUE, data.table = F)
ex_clean <- ex_clean.raw[, -1]

hist(as.matrix(ex_clean))
